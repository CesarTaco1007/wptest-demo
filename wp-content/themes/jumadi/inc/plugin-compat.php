...
add_action( 'after_setup_theme', 'jumadi_setup_woocommerce' );
/**
 * Set up WooCommerce
 *
 */
function jumadi_setup_woocommerce() {
	...
	add_action( 'woocommerce_sidebar', 'jumadi_construct_sidebars' );
}

if ( ! function_exists( 'jumadi_woocommerce_start' ) ) {
	add_action( 'woocommerce_before_main_content', 'jumadi_woocommerce_start', 10 );
	/**
	 * Add WooCommerce starting wrappers
	 *
	 */
	function jumadi_woocommerce_start() { ?>
...
if ( ! function_exists( 'jumadi_woocommerce_end' ) ) {
	add_action( 'woocommerce_after_main_content', 'jumadi_woocommerce_end', 10 );
	/**
	 * Add WooCommerce ending wrappers
	 *
	 */
	function jumadi_woocommerce_end() { ?>
						</div><!-- .entry-content -->
						<?php
						/**
...
if ( ! function_exists( 'jumadi_woocommerce_css' ) ) {
	add_action( 'wp_enqueue_scripts', 'jumadi_woocommerce_css', 100 );
	/**
	 * Add WooCommerce CSS
	 *
	 */
	function jumadi_woocommerce_css() {
		if ( ! class_exists( 'WooCommerce' ) ) {
			return;
		}

		$mobile = apply_filters( 'jumadi_mobile_media_query', '(max-width:768px)' );
...
if ( ! function_exists( 'jumadi_bbpress_css' ) ) {
	add_action( 'wp_enqueue_scripts', 'jumadi_bbpress_css', 100 );
	/**
	 * Add bbPress CSS
	 *
	 */
	function jumadi_bbpress_css() {
...
if ( ! function_exists( 'jumadi_buddypress_css' ) ) {
	add_action( 'wp_enqueue_scripts', 'jumadi_buddypress_css', 100 );
	/**
	 * Add BuddyPress CSS
	 *
	 */
	function jumadi_buddypress_css() {
...
if ( ! function_exists( 'jumadi_beaver_builder_css' ) ) {
	add_action( 'wp_enqueue_scripts', 'jumadi_beaver_builder_css', 100 );
	/**
	 ...
	 *
	 */
	function jumadi_beaver_builder_css() {
