...
if ( class_exists( 'WP_Customize_Control' ) && ! class_exists( 'Jumadi_Typography_Customize_Control' ) ) {
	/**
	 * Create the typography elements control.
	 *
	 */
	class Jumadi_Typography_Customize_Control extends WP_Customize_Control {
		public $type = 'jumadi-customizer-typography';

		public function enqueue() {
			wp_enqueue_script( 'jumadi-typography-selectWoo', trailingslashit( get_template_directory_uri() )  . 'inc/customizer/controls/js/selectWoo.min.js', array( 'customize-controls', 'jquery' ), JUMADI_VERSION, true );
			wp_enqueue_style( 'jumadi-typography-selectWoo', trailingslashit( get_template_directory_uri() )  . 'inc/customizer/controls/css/selectWoo.min.css', array(), JUMADI_VERSION );

			wp_enqueue_script( 'jumadi-typography-customizer', trailingslashit( get_template_directory_uri() )  . 'inc/customizer/controls/js/typography-customizer.js', array( 'customize-controls', 'jumadi-typography-selectWoo' ), JUMADI_VERSION, true );
			wp_enqueue_style( 'jumadi-typography-customizer', trailingslashit( get_template_directory_uri() )  . 'inc/customizer/controls/css/typography-customizer.css', array(), JUMADI_VERSION );
		}

		public function to_json() {
			parent::to_json();

			$number_of_fonts = apply_filters( 'jumadi_number_of_fonts', 200 );
			$this->json[ 'default_fonts_title'] = __( 'System fonts', 'jumadi' );
			$this->json[ 'google_fonts_title'] = __( 'Google fonts', 'jumadi' );
			$this->json[ 'google_fonts' ] = apply_filters( 'jumadi_typography_customize_list', jumadi_get_all_google_fonts( $number_of_fonts ) );
			$this->json[ 'default_fonts' ] = jumadi_typography_default_fonts();
			$this->json[ 'family_title' ] = esc_html__( 'Font family', 'jumadi' );
			$this->json[ 'weight_title' ] = esc_html__( 'Font weight', 'jumadi' );
			$this->json[ 'transform_title' ] = esc_html__( 'Text transform', 'jumadi' );
			$this->json[ 'category_title' ] = '';
			$this->json[ 'variant_title' ] = esc_html__( 'Variants', 'jumadi' );
		...
		public function content_template() {
			...
			<# if ( 'undefined' !== typeof ( data.variant ) ) { #>
				...
				#>
				<div id={{{ data.variant.id }}}" class="jumadi-font-variant" data-saved-value="{{ data.variant.value }}">
					<label>
			...
			<# if ( 'undefined' !== typeof ( data.category ) ) { #>
				<div class="jumadi-font-category">
					<label>
							<input name="{{{ data.category.id }}}" type="hidden" {{{ data.category.link }}} value="{{{ data.category.value }}}" class="jumadi-hidden-input" />
			...
			<# if ( 'undefined' !== typeof ( data.transform ) ) { #>
				<div class="jumadi-font-transform">
					<label>
