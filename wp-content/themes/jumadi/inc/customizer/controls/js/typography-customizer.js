( function( api ) {
	...
	api.controlConstructor['jumadi-customizer-typography'] = api.Control.extend( {
		ready: function() {
			var control = this;

			control.container.on( 'change', '.jumadi-font-family select',
				function() {
					...
					setTimeout( function() {
						// Send our request to the jumadi_get_all_google_fonts_ajax function
						var response = jQuery.getJSON({
							type: 'POST',
							url: ajaxurl,
							data: {
								action: 'jumadi_get_all_google_fonts_ajax',
								jumadi_customize_nonce: jumadi_customize.nonce
							},
							async: false,
							dataType: 'json',
						});
						...
						if ( id in fonts ) {
							...
							jQuery( '.jumadi-font-family select' ).not( _this ).each( function( key, select ) {
								var parent = jQuery( this ).closest( '.jumadi-font-family' );

								if ( _value == jQuery( select ).val() && _this.data( 'category' ) !== jQuery( select ).data( 'category' ) ) {
									if ( ! got_variants ) {
										updated_variants = jQuery( parent.next( '.jumadi-font-variant' ).find( 'select' ) ).val();
										got_variants = true;
									}
								}
							} );
							...
							_this.closest( '.jumadi-font-family' ).next( 'div' ).show();
							...
							_this.closest( '.jumadi-font-family' ).next( 'div' ).hide();
			...
			control.container.on( 'change', '.jumadi-font-variant select',
				function() {
					var _this = jQuery( this );
					...
					jQuery( '.jumadi-font-variant select' ).each( function( key, value ) {
						var this_control = jQuery( this ).closest( 'li' ).attr( 'id' ).replace( 'customize-control-', '' );
						var parent = jQuery( this ).closest( '.jumadi-font-variant' );
			...
			control.container.on( 'change', '.jumadi-font-category input',
				function() {
					control.settings['category'].set( jQuery( this ).val() );
				}
			);

			control.container.on( 'change', '.jumadi-font-weight select',
				function() {
					control.settings['weight'].set( jQuery( this ).val() );
				}
			);

			control.container.on( 'change', '.jumadi-font-transform select',
				function() {
...
jQuery( document ).ready( function($) {
	...
	jQuery( '.jumadi-font-family select' ).selectWoo();
	jQuery( '.jumadi-font-variant' ).each( function( key, value ) {
		var _this = $( this );
	...
	$( ".jumadi-font-family" ).each( function( key, value ) {
		var _this = $( this );
		if ( $.inArray( _this.find( 'select' ).val(), typography_defaults ) !== -1 ) {
			_this.next( '.jumadi-font-variant' ).hide();
		}
	});
